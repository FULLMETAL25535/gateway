...
struct std_msg
{
    long msg_type; //必须有，表示消息的类型，1：stm32 2: modbus
    int key;       //设备key值
    char val[32];  //数值转为string类型，拿到数据注意转化为其他类型再使用
};
...
//发信息到消息队列
void mycontrol(int key, char *string);

/* TODO: 替换为自己设备的三元组 */
const char *product_key = "k1l8eke1Pbb";
const char *device_name = "Device1";
const char *device_secret = "feb6958d1928c85dae932a5e51bfd3e1";
...
*/
const char *mqtt_host = "iot-06z009y4n6tjpam.mqtt.iothub.aliyuncs.com";
/* 
...
static void demo_dm_recv_property_set(void *dm_handle, const aiot_dm_recv_t *recv, void *userdata)
{
    ...
    sprintf(buf, "%.*s",
            recv->data.property_set.params_len,
            recv->data.property_set.params);
    cJSON *set = cJSON_Parse(buf);
    char key[32] = "";
    char val[32] = "";
    for (int i = 0; i < datasize; i++)
    {
        sprintf(key, "%d", p->key);
        cJSON *item = cJSON_GetObjectItem(set, key);
        if (item != NULL)
        {
            struct std_msg mb;
            mb.key = p->key;
            if (mb.key > 300)
            {
                mb.msg_type = 10; //stm32
            }
            else
            {
                mb.msg_type = 5; //modebus
            }
            printf("%d\n", p->dev_type);
            if (p->type == 1)
            {
            ...
            {
                sprintf(val, "%f", item->valuedouble);
                strcpy(mb.val, val);
            }
            printf("%ld %d %s\n",mb.msg_type,mb.key,mb.val);
            msgsnd(msgid, &mb, sizeof(struct std_msg) - sizeof(long), 0);
        }
        p++;
    }
    cJSON_Delete(set);
...
int main(int argc, char *argv[])
{
    ...
    struct std_node *p;
    //  printf("line 589\n\n");
    //  demo_send_property_post(dm_handle, "{\"Light_Switch\": 1}");
    /* 主循环进入休眠 */
    while (1)
    {
        // printf("line 593\n");
        ...
        for (int i = 0; i < datasize; i++)
        {
            if (p->type == 1)
            {
                sprintf(buf, "{\"%d\": %d}", p->key, p->new_val.b_val);
            }
            else if (p->type == 2)
            {
                sprintf(buf, "{\"%d\": %d}", p->key, p->new_val.i_val);
            }
            else if (p->type == 3)
            {
                sprintf(buf, "{\"%d\": %.2f}", p->key, p->new_val.f_val);
            }
            else
            {
                // printf("p-type 有问题!:%d\n", p->type);
...
int shm_init()
{
    key_t key;
    char path[256];
    sprintf(path,"/tmp/ipc/shmem/%s", SHMID);
    ...
    shmid = shmget(key, sizeof(struct std_msg)+4, IPC_CREAT | IPC_EXCL | 0666);
    if (shmid < 0)
    {
        if (errno == EEXIST)
        {
            shmid = shmget(key, sizeof(struct std_msg), 0666);
        }
        else
        {
            perror("shmget err");
